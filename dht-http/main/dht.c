...
void dht_task(void *pvParameters) {
    while (1) {
        ...
        dht_data_t dht;
        ESP_LOGI(TAG, "Retrieving temp and humidity");
        if (dht_read_float_data(DHT_TYPE_DHT11, GPIO_NUM_25, &humidity, &temperature) == ESP_OK){
            ...
            xQueueSend(dht_queue, &dht, portMAX_DELAY);
...
void http_task(void *pvParameters) {
    ...
    xQueueReceive(dht_queue, &dht_received, portMAX_DELAY);
    char body[50];
    sprintf(body, "{\"temp\": %.2f, \"humidity\": %.2f}", dht_received.temp, dht_received.hum);
    esp_http_client_set_post_field(client, body, strlen(body));
    esp_http_client_set_header(client, "Content-Type", "application/json");
    esp_err_t err=esp_http_client_perform(client);
    if (err== ESP_OK){
        ESP_LOGI(TAG, "humidity SENT %.2f and temp sent %.2f", dht_received.hum, dht_received.temp);
        vTaskDelay(5000/portTICK_PERIOD_MS);
    }
    }
}
...
void app_main(void)
{
    ...
    xTaskCreate(&dht_task, "dht_task", 2048, NULL, 4, NULL);
